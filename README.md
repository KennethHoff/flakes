...
`pkgs/<tool>/` directory; the root `flake.nix` auto-discovers them, so adding a
...
| --- | --- | --- |
| `aspire-cli` (+ `.stable`/`.staging`/`.dev`) | [Aspire CLI](https://aspire.dev) | [`pkgs/aspire`](pkgs/aspire) |
| `playwright-cli` | [Playwright CLI](https://playwright.dev) | [`pkgs/playwright`](pkgs/playwright) |
| `sentry-cli` | [Sentry CLI](https://cli.sentry.dev) | [`pkgs/sentry`](pkgs/sentry) |
...
1. Create `pkgs/<tool>/` and add the packaging files (`package.nix`,
   `versions.nix`, `update.sh`, `README.md`, optional `tests/`).
2. Add `pkgs/<tool>/default.nix` returning the tool-module contract — every key
   is optional:
   ...
   in {
     name = "<name>";                                           # primary package = commit scope
     ...
     # `update-<dir>` app for you (adds git + a cd into this dir):
   ...
   `<name>` is whatever the tool is called (`aspire-cli`, `sentry-cli`, … — the
   `-cli` suffix is incidental, not required). Channels/variants can hang off a
   package as sub-attrs (`aspire-cli.staging`), runnable via the same
   mainProgram fallback.
...
`update-<tool>` app. The only reason to touch the root `flake.nix` is when a
tool needs an extra **flake input** (Nix requires inputs at the root) — see the
`nixpkgs-openssl` pin that `pkgs/aspire` consumes.

### Conventions a tool must follow

- **Unique output names** — packages/apps share one namespace across tools.
- **Declared `name`** — each module sets `name` (its primary package). The root
  surfaces it as `lib.packageNames`, and the update workflow uses it as the
...
- **Namespaced checks** — `tests/default.nix` returns bare keys; the root
  prefixes them with `<tool>-`.

## Updating versions

Each tool ships an `update-<tool>` app that bumps its `versions.nix`. Run it
from anywhere in the repo:

```bash
nix run .#update-sentry
```
...
- **`nixpkgs-openssl` pin.** Temporary OpenSSL 3.6.1 pin for the Aspire CLI;
  carries a `TODO(~2026-07)` to revisit — see `pkgs/aspire/default.nix`.
