{
  description = "Monorepo of personal Nix flakes — one packaged CLI per pkgs/<tool>/ directory";

  inputs = {
    nixpkgs.url = "github:nixos/nixpkgs/nixos-unstable";

    # Consumed only by pkgs/aspire (OpenSSL 3.6.1 pin — see its default.nix).
  ...
  outputs =
    ...
    let
      ...
      # A tool's declared `name` is system-independent; read it off any system.
      firstSystem = builtins.head systems;

      # Auto-discovery: every subdirectory of ./pkgs is a tool. Adding a CLI is
      # "drop a pkgs/<tool>/default.nix" — no edit to this file (unless the tool
    ...
    in
    {
      ...
      # Maps each tool dir -> its declared primary package name. The auto-update
      # workflow reads this to scope its commits as `chore(<name>): ...`. Falls
      # back to the directory name if a tool omits `name`.
      lib.packageNames = lib.genAttrs toolNames (name: toolOutputs.${firstSystem}.${name}.name or name);
    };
}
